...
页面右上角有4个按钮，分别是【读取reactions.csv】、【读取reactants.csv】、【读取environment_effects.csv】、【读取attack_types.csv】按钮
按钮下方一行对应四个tab，分别是【reactions】、【reactants】、【environment_effects】、【attack_types】
点击tab切换时，会切换到对应的表格中。
页面主体部分是一个表格，表格的列名和reactions.csv文件保持一致。
表格的宽度为屏幕宽度的90%。
表格行数过多时进行分页展示，每一页20行。
表格上方有【新增xx】【保存文件】、【复制csv文本】【自动排序】按钮
【新增】按钮。
...
```json
{
    ...
    "attack_types_file": "attack_types.csv"
}
```
...
6、【自动排序】
6-1、点击【自动排序】按钮后，根据id列对表格数据进行排序。
6-2、排序后，id列的数值应该是连续的，从1开始递增。
